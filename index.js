...
function lifeInWeeks() {
  ...
  document.querySelector("h1").textContent =
    ...
    " weeks, and " +
    monthsLeft +
    " months left if you live until 90.";
}
...
function bmiCalculator() {
  ...
  document.querySelector("h1").textContent =
    "Your BMI is " + roundedBmi + ". You are " + status + ".";
}
...
function loveCalculator() {
  ...
  document.querySelector("h1").textContent =
    "Your love score is: " + percent + "%. " + loveMessage;
}
...
function beerBottleSong() {
  ...
  while (i >= 0) {
    if (i === 0) {
      console.log(
        "No more bottles of beer on the wall. No more bottles of beer. Go to the store, buy some more. 100 bottles of beer on the wall."
      );
    } else {
      console.log(
        i +
...
document.querySelector(".go-btn").addEventListener("click", showResult);
function showResult() {
  var userText = document.getElementsByClassName("user-input")[0];
  var userCalculation = userText.value;
  findCalculation(userCalculation);
  if(/\d/.test(userText)){
    //vallid!!
  }else{
    //invalid!!
...
function findCalculation(userCalculation) {
  if (!userCalculation) {
    document.querySelector("h1").textContent =
      "Welcome to Melanie's calculator.";
  } else {
    for (i = 0; i < userCalculation.length; i++) {
      var userChar = userCalculation.charAt(i);
      ...
      if (isNaN(userChar)) {
        ...
        if (
          ...
          console.log("removing invisibility.");
          break;
        }
      }else{
        document.querySelector(".error-message").classList.add("invisible");
      }
      console.log(userCalculation[1]);
